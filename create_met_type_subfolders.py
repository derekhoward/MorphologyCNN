...
types = set()
for _, row in df.iterrows():
    if row["MET type"] in sig_cell_types and os.path.exists(f"./gouwens-data/preprocessed_images/{row['Specimen ID']}.png"):
...
train_dir = f"./gouwens-data/training_images_met_type"
test_dir = f"./gouwens-data/test_images_met_type"
...
for cell, label in zip(cells, labels):
    ...
    if type_freq[label] > test_cell_count:
        dst = f"./gouwens-data/training_images_met_type/{label}/{cell}.png"
    else:
        # Save 2 images per class label for test set
        dst = f"./gouwens-data/test_images_met_type/{label}/{cell}.png"
