...
from vanilla_cnn_subclass import VanillaCNN
...
def main():
    ...
    model_path = "gouwens-data/models/vanilla_cnn_gouwens_subclass_b2_lr1e-4_e7_rs287"
    ...
    metadata = pd.read_csv("./combined-data/combined_metadata.csv")

    subclasses = ['Lam', 'Pva', 'Snc', 'Sst', 'Vip']

    test_cells = {}
    for _, row in metadata.iterrows():
        if row["dataset"] == "scala" and row["t-type"][:3] in subclasses:
            test_cells[row["cell_id"]] = subclasses.index(row["t-type"][:3])
    ...
    for cell, label in test_cells.items():
        try:
            ...
            if subclasses[label] not in cell_label_pred:
                cell_label_pred[subclasses[label]] = [(cell, pred, label_rank)]
            else:
                cell_label_pred[subclasses[label]].append((cell, pred, label_rank))
        except:
    ...
    for label, v in cell_label_pred.items():
        ...
        # Uncomment to print correct, wrong, and rank of predictions per subclass
    ...
    cm = confusion_matrix(labels, predictions, labels=list(range(len(subclasses))))
    cmn = (cm.astype('float') + 0.00001 * cm.astype('float')) / (cm.sum(axis=1) + 0.00001)[:, np.newaxis]
    fig, ax = plt.subplots(figsize=(12, 10))
    hm = sns.heatmap(cmn, yticklabels=subclasses)
    hm.set_xticklabels(subclasses, size=17)
    hm.set_yticklabels(subclasses, size=17)
    cbar = hm.collections[0].colorbar
    cbar.ax.tick_params(labelsize=17)
    plt.subplots_adjust(bottom=0.28, left=0.28)
    plt.savefig("./scala-data/plots/inference_subclass_modified.png")
