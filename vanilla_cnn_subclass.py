...
class VanillaCNN(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(8 * 27 * 27, 256)
...
def plot_results(num_epochs, y_train, y_valid, metric, outfile):
    ...
    plt.title(f"{metric} for Vanilla CNN")
    plt.xlabel("Epochs")
    plt.ylabel(metric)
    plt.legend()
    plt.savefig(outfile)
...
def train(input_dir, model, loss_fn, optimizer, batch_size, num_epochs, augment=False, test_dir=None, test_batch_size=None):
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(valid_loader):
            ...
            if epoch == num_epochs - 1:
                for i in range(len(labels)):
                    ...
                    cm_labels.append(labels[i].float())
                    cm_outputs.append(np.argmax(output.data[i].flatten()))
        ...
        print(f"\tValidation MRR: {v_mrr_sum / v_mrr_count}")
        ...
        if epoch == num_epochs - 1:
            ...
            plt.savefig("./gouwens-data/plots/vanilla_cm_subclass_modified.png")
    
    print(f"Training time: {round(time() - time_start, 1)}s")

    # Uncomment below to plot accuracy and loss over epochs
    # plot_results(num_epochs, train_acc, valid_acc, "Accuracy", "./combined-data/plots/vanilla_accuracy_subclass.png")
    # plot_results(num_epochs, train_loss, valid_loss, "Loss", "./combined-data/plots/vanilla_loss_subclass.png")

    # Uncomment below to save model weights
    # model_path = './gouwens-data/models/vanilla_cnn_gouwens_subclass_b2_lr1e-4_e6_rs28'
...
def main():
    ...
    input_dir = "./gouwens-data/training_images_subclass_modified/"
    ...
    # test_dir = "./combined-data/test_images_subclass/"
